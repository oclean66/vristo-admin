...
<div x-data="alert" x-show="open">
    <div
        {{ $attributes->merge(['class' => 'flex items-center p-3.5 rounded text-' . $color . ' bg-' . $color . '-light dark:bg-' . $color . '-dark-light']) }}>
        <span class="ltr:pr-2 rtl:pl-2"><strong
                class="ltr:mr-1 rtl:ml-1">{{ $title }}</strong>{{ $description }}</span>
